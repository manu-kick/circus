...
int main() {
    ...
    while(!glfwWindowShouldClose(sim.window)) {
        ...
        Vector4d quat;
        for (int j = 0; j < 4; ++j) {
            quat[j] = sim.mujData->xquat[4+j]; // since the body index is 1
        }
        // std::cout << "Body quaternion: " << quat[0] << " , " << 
        //                                     quat[1] << " , " << 
        //                                     quat[2] << " , " << 
        //                                     quat[3] << std::endl;
        // std::cout << "Lin Vel: " << sim.mujData->cvel[9+0] << " , " <<
        //                             sim.mujData->cvel[9+1] << " , " <<
        //                             sim.mujData->cvel[9+2] << std::endl;
        // std::cout << "Ang Mom: " << sim.mujData->cvel[6+0] << " , " <<
        //                             sim.mujData->cvel[6+1] << " , " <<
        //                             sim.mujData->cvel[6+2] << std::endl;
